...
def preprocess_text(genuine, clickbait):
    ...
    glove_vocabulary = open("data/vocabulary.glove.txt", encoding="utf8").read().split("\n")
...
if __name__ == "__main__":
    genuine = open("data/genuine.txt", encoding="utf8").read()
    clickbait = open("data/clickbait.txt", encoding="utf8").read()
    vocabulary, genuine_preprocessed, clickbait_preprocessed = preprocess_text(genuine, clickbait)
    open("data/vocabulary.txt", "w").write("\n".join(vocabulary))
    open("data/genuine.preprocessed.txt", "w", encoding="utf8").write(genuine_preprocessed)
    open("data/clickbait.preprocessed.txt", "w", encoding="utf8").write(clickbait_preprocessed)